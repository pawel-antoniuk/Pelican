...
bool HTTPPostRequest::on_message(const std::string& data, IMessageSender* sender, IMetadata* meta){
	char type[8];
	char buf[255];
	sscanf_s(data.c_str(), "%s %s HTTP/1.1\r\n", type, sizeof(type), buf, sizeof(buf));

	bool state = false;
	if (!dynamic_cast<SingleData<std::string>*>(meta)->data.compare("POST")){
